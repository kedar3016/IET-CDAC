...
public class ProductDaoImpl implements ProductDao{
	...
	public Product findByPid(int pid) {
		...
		try {
			Product p=jdbctemplate.queryForObject("select * from product1 where pid=?",
	...
	public boolean modifyProduct(Product p) {
		int n = jdbctemplate.update("update product1 set pname=?,qty=?,price=?,cid=? where pid=?",new Object[] {p.getPname(),p.getQty(),p.getPrice(),p.getCid(),p.getPid()});
		return n>0;
	}	
}
